...
txtlog version one point two by daniel gatenio
http://gaten.io/
*/

//initialise our friend, textlog
 $t = new txtlog($_GET['url']);
/************************************************
...
function __construct($URL){
	//take the input from the request
	$this->requestURL = $URL;
	...
	$this->files = glob($this->requestPath ."*.{jpg,jpeg,gif,png,md,txt,mp3}",GLOB_BRACE);//get all the files txtlog can handle
	...
	usort($this->everything, create_function('$c,$d', 'return filemtime($d) - filemtime($c);'));
	usort($this->special, create_function('$c,$d', 'return filemtime($d) - filemtime($c);'));
	usort($this->files, create_function('$c,$d', 'return filemtime($d) - filemtime($c);'));
	}
				...
				if($filename !== about){
					...
					if(is_dir($thisSpecial)){
						if(preg_match("/hyperesources$/",$thisSpecial) !== 1){
							$html .= '<div title="' . $hoverText . '" class=folder> <a href="' . str_replace(' ', '-', ltrim($thisSpecial, 'txt')) . '">' .  $filename . '</a></div>';//make a div, urlify the folder, end the div
					}
				}
				else{
		...
		foreach($this->files as $thisFile){
			...
			if($filename !=='top'){
				...
				switch($ext){
					...
					case 'gif':
					case 'png':
						$html .='<img max-width:500 title="' . $hoverText .'" src="/' .$thisFile . '" /></div>';//format the images
						break;
						
					case 'txt':
					case 'md':
						$html .= '<div title="' . $hoverText . '" class="article articlecolor">'; //make the container
					...
					case 'folder':
						if(preg_match("/hyperesources$/",$thisFile) !== 1){
							$html .= '<div title="' . $hoverText . '" class=folder> <a href="' . str_replace(' ', '-', ltrim($thisFile, 'txt')) . '">' .  $filename . '</a></div>';//make a div, urlify the folder, end the div
						}
						break;
